pipeline {
    ...
    environment {
        DOCKER_IMAGE_NAME = "cjy951213/ecommerce-fe-admin-image"
        ...
        stage('Build Image') {
            steps {
                script {
                    ...
                    sh "docker build -t ${DOCKER_IMAGE_NAME}:latest ecommerce-admin"
                }
            }
        }
            ...
            steps {
                ...
                sshagent(credentials: [REMOTE_SSH_CREDENTIAL_ID]) {
                    sh """
                        ...
                        docker stop ecommerce-fe-admin || true
                        docker rm ecommerce-fe-admin || true
                        
                        echo "--- (원격) 3. 새 컨테이너 실행 ---"
                        docker run -d -e TZ=Asia/Seoul -p 4000:4000 --name ecommerce-fe-admin ${DOCKER_IMAGE_NAME}:latest
